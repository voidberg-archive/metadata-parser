  ...
  var UTF8={
    ...
    'getCharCode': function(bytes, byteOffset, charLength) {
      ...
      if(bytes[byteOffset]&(parseInt(mask,2))) {
        throw new Error('Index '+byteOffset+': A '+charLength+' bytes encoded char' + ' cannot encode the '+charLength+'th rank bit to 1.');
      }
      ...
      while(charLength) {
        if(0x80!==bytes[byteOffset+1]&0x80 || 0x40===bytes[byteOffset+1]&0x40) {
          throw new Error('Index '+(byteOffset+1)+': Next bytes of encoded char must begin with a "10" bit sequence.');
        }
    ...
    'getStringFromBytes': function(bytes, byteOffset, byteLength, strict) {
      ...
      for(; byteOffset<byteLength; byteOffset++) {
        ...
        if(byteOffset+charLength>byteLength) {
          if(strict) {
            throw new Error('Index '+byteOffset+': Found a '+charLength+' bytes' + ' encoded char declaration but only '+(byteLength-byteOffset) + ' bytes are available.');
          }
        } else {
